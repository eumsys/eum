...
import time
############# ARDUINO ###################################
import leerBotonesArduino as botones
##########################################################
				...
				if botones.leerMasa(arduino) == True:
					...
					if self.f1 == True and self.f2 == False and self.f3 == False and self.f4 == False and self.f5 == False:
						if boleto_previo == False:
							...
							print("ID IMPRESORA EXPEDIDORA: {}".format(idImpresora))
							folioBD = noBolF
							fechaIn	= fechaUTC.fechaConFormato()
							horaEnt = fechaUTC.tiempoConFormato()
							...
							if validacion == "error":
								...
								if valor == "error":
									...
									self.inicioImpresion()
									respuesta = imprimir.imprimirQR2(idImpresora,noBolF,idImpresora,str(fechaIn),str(horaEnt))
