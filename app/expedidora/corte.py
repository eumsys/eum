...
def imprimirCorte(expedidos,folioInicial,folioFinal,turno,encendidos,apagados,num):
	print("emp")
	try:
		Generic = instanciarImpresora()
		if Generic == "No esta conectada la impresora":
...
ultimoFolio=infile.readline()
print('ufff',ultimoFolio)
infile.close()
...
folioInicial=str(arr[1])
folioFinal=str(ultimoFolio)
turno=str(arr[2])
expedidos=str(int(folioFinal)-int(arr[1]))
infile.close()

infile = open("/home/pi/Documents/eum/app/expedidora/datos/logs.txt","r")
datoss=infile.readline().split(',')
encendidos=str(datoss[0])
apagados=str(datoss[1])
infile.close()
#imprime= commands.getoutput("lp -d SANEI_SK_Series /home/pi/Documents/eum/impresora/pruebas/test.pdf")
#imprimirHeader()
imprimirCorte(expedidos,folioInicial,folioFinal,turno,encendidos,apagados,num)
