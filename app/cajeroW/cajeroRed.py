...
def interface():
	class Ventana(QDialog):
		...
		def __init__(self):
			...
			gui = uic.loadUi(ruta+"../Interfaces/CRB-4.2.1.ui", self)
	...
	################MODS########
		...
		def montos(self):
			...
			if(conexion_activa):
				...
				#time.sleep(4)
				#os.system("./Documents/eum/pushNotify2.sh 'eum.conf'")
...
def aceptarBilletes(estadoConexion,TON):
	...
	if(rBill):
		if(rBill[0]==144 or rBill[0]==145 or rBill[0]==146 or rBill[0]==147 or rBill[0]==148):
			...
			accept_sequence(ser,estadoConexion)
			count(ser,estadoConexion)
			#if(aux_cambio<0):
			#	enable_coin(ser)
			#	enable_coin(ser)
...
def aceptarMonedaManual(estadoConexion,r):
	global cp,cambio,tarifa,aux_cambio,aux,rep,ser,aux_tarifa,killbill
	'''
	ser.flushOutput()
	ser.flushInput()
...
def habilitarDispositivosCobro(estadoConexion):
	global ESTADO_BILLETERO,tarifaVoluntaria,cp,tiempoAgotadoDePago,cambiaColor,total,bill,cambio,tarifa,aux_cambio,aux,rep,estatus,ser,killbill,aux_tarifa,bill,pagado,billetesTotales,dineroTotal,billetes,dineroTotalB,billetesPago
...
def enable_sequence(ser):
	...
	while(1):
		...
		TON_01.actualizar()
		print("tiempo transcurrido: ",TON_01.tiempoActual,TON_01.salida,TON_01.tiempo)
		if TON_01.salida:
			print("enable_sequence failed: no se puede habilitar el billetero")
			return ask
...
def accept_sequence(ser, estadoConexion):
	...
	while(1):
		...
		if(r):
			...
			if(aux.bin[0:4]=="1000"):
				...
				print("PM...")
				#aceptarMonedaManual(estadoConexion,r)
...
def registrarPagoActual(cp, NoCajero, aux_cambio, aux_tarifa, costillo, datosOperacionBD, estadoConexion, descripcionMonedas, descripcionBilletes, descripcionMonedasCambio, tipo, CODIGO_ERROR):
	...
	try:
		...
		cur.execute(consu)
		conn.commit()
		print("Se registro el pago en BD interna. ",datosOperacionBD)
	except:
...
if __name__ == "__main__":
	...
	#threadMonedas = Thread(target=leerMonedas, args=())
	#threadBilletes = Thread(target=leerBilletes, args=())
...
#Setup Uart https://www.circuits.dk/setup-raspberry-pi-3-gpio-uart/
