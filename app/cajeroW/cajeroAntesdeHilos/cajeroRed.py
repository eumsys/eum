				...
				if(str("M") in str(folio)):
					...
					if(folio == 'boleto perdido'):
						...
						if(A!=-1):
							...
							consu="insert into \"BOLETO\"(tarifa,cajero,tipo,estado,pago,expedidora,\"fechaExpedicion\") values("+str(tarifaSeleccionada)+","+str(NoCajero)+",1,"+str(estadoBoleto)+",1,'"+str(pe)+"','"+fe+"')"
							cur.execute(consu)
							conn.commit()
							while(aux_tarifa==0):
								time.sleep(.5)
...
def count(ser):
	...
	if(registraPago==1):
		...
		#REGISTRA PAGO EN BD.......
		consu="insert into \"PAGOS\"(\"idBoleto\",expedidora,\"fechaExpedicion\",codigo,conexion,monto,cambio,monedas,billetes, cambio_entregado, tipo) values("+str(fo)+","+str(pe)+","+"'"+str(fechaAMD)+" "+str(h)+"'"+","+str(0)+","+str(0)+","+str(aux_tarifa)+","+str(aux_cambio)+",'"+str(descripcionMonedas)+"','"+str(descripcionBilletes)+"','"+str(descripcionMonedasCambio)+"',"+str(0)+")"
		cur.execute(consu)
		conn.commit()
		"""
		...
		g=Servidor.configSocket("pago boleto", datosOperacion+"*"+mensaje)
		if(g==-1):
			Sinpago=Sinpago+1
