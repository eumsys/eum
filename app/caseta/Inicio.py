...
from PyQt5.QtWidgets import QWidget,QMainWindow,QApplication, QDialog, QGridLayout, QMessageBox, QLabel, QPushButton, QLineEdit,QSpinBox, QTableWidget,QTableWidgetItem,QComboBox,QCheckBox
...
MODO_OPERACION = 2
INTERFAZ_COBRO_TARIFA = 23
INTERFAZ_COBRO_DEPOSITAR = 18
INTERFAZ_COBRO = INTERFAZ_COBRO_TARIFA
def interface():
	...
	class Ventana(QDialog):
		...
		def __init__(self):
			...
			self.bntarifa.clicked.connect(lambda:self.cambia(12))
			self.bcancelar.clicked.connect(lambda:self.cambia(11))
			self.bsalirTarifas.clicked.connect(lambda:self.cambia(0))
			...
			self.salirplaza.clicked.connect(lambda:self.cambia(0))
			...
			self.salirReportes.clicked.connect(lambda:self.cambia(0))
			...
			self.breporte.clicked.connect(lambda:self.cambia(17))
...
#			self.bimpReporteCaja.clicked.connect(self.imprimeReporte)
			...
			self.bvolverPropina.clicked.connect(lambda:self.cambia(20))
			self.bpanelconf.clicked.connect(lambda:self.cambia(4))
			...
			self.bcine.clicked.connect(lambda:self.abrirBarrera(6))
			...
			self.tablatarifas.setColumnCount(8)
			self.tablatarifas.setHorizontalHeaderLabels(['Estado','Id','Prioridad','Descripcion','costo','intervalo_1','intervalo_2','Costo 2'])
			...
			self.bcosto1.clicked.connect(lambda:self.abrirBarrera(6))
			self.bcosto2.clicked.connect(lambda:self.abrirBarrera(9))
	...
	################MODS########
		...
		def setConfig(self):
			...
			infile = open("/home/pi/Documents/eum/app/caseta/archivos_config/datos.txt", 'w')
			c=infile.write(dat)
		...
		def cambiaIp(self):
			...
			self.sustituye("/home/pi/Documents/eum/app/caseta/cliente.py","192.168","host = '"+host+"'")
			...
			print(plaza,localidad)
			dat=host+","+ip
			infile = open("/home/pi/Documents/eum/app/caseta/archivos_config/red.txt", 'w')
			c=infile.write(dat)
		...
		def todobien(self):
			...
			outfile = open("/home/pi/Documents/eum/app/caseta/archivos_config/fechaDeCorte.txt", 'w')
			...
			fechaUltimoCorte=fec+" "+horaHoy
			print("Hora ultimo corte: ", fechaUltimoCorte)
			if(str(turno)=='M'):
				turno='V'
			else:
				turno='M'
			folio=int(folio)+1
			outfile.write(str(fechaUltimoCorte)+","+str(turno)+","+str(folio))
			outfile.close()
			self.cambia(0)
			
				
				
		def verificarPropina(self,propina):
			global val
			print("PROPPP",val)
			if(val==0):
				self.cambia(19)
		...
		def ticketsino(self,sino):
			global val,contador,c,aux_cambio,total,w,killer,aux_tarifa,aux_tarifa2,hh
			if(sino == 1):
		...
		def abrirBarrera(self,tipo):
			...
			1;"Cliente";"usuario del estacionamiento"
			2;"Cortesia";"locatarios, gerentes, pensiones"
			3;"Perdido";"boletos perdidos"
			4;"Tolerancia";"sin pago dentro de 0 a 15 min"
			5;"con propina";""
			6;"Sellados completos";""
			7;"Sellados incompletos";""
			8;"Sin propina";""
			9;"No sellados completos";"No sellados completos"
			10;"No sellados";"No sellados incompletos"

			"""
			global contador,cobroIniciado,pagado,tarifaSeleccionada,NoCajero,pe,fe,costillo,hh,fo,mensajeBoletoUsado,aux_tarifa,aux_tarifa2,costo2
			pagado=2
			horaHoy=time.strftime("%H:%M:%S")
			fecha=datetime.now().date()
		...
		def cancelandoPago(self):
			global val,cp,aux_tarifa,aux_tarifa2,aux_tarifa1,total,killer,cobroIniciado
			cobroIniciado=1
			self.cambia(0)
			aux_tarifa = 0
			costo2=0
		...
		def llenaCamposPlaza(self):
			...
			self.lloc.setText(loc)
			self.cambia(15)

		def cambiaNombre(self):
			global nom,loc,USUARIO,val
			fecha=hora.mostrarFechayHora()
		...
		def tecladoSum(self,val):
			...
			if(val==10):
				...
				self.valvol3.setText(self.valvol3.text()+str(val))
				
			self.desactivaTeclado()
			tiempoTeclado=1

		def volConfirmado(self):
			global aux_tarifa,costo2
		...
		def llenaTabla(self):
			global cur
			cur.execute("select \"idTarifa\",des_tar,costo,int_1,int_2,estado,prioridad,descuento from \"TARIFA\" order by costo Asc")
			...
			for reg in cur:
				...
				des=QTableWidgetItem(str(reg[1]))
				cos=QTableWidgetItem(str(reg[2]))
				i1=QTableWidgetItem(str(reg[3]))
				i2=QTableWidgetItem(str(reg[4]))
				pri=QTableWidgetItem(str(reg[6]))
				descuento=QTableWidgetItem(str(reg[7]))
				if(reg[5]==1):
				...
				self.tablatarifas.setItem(row,3,des)
				self.tablatarifas.item(row,3).setTextAlignment(4)
				self.tablatarifas.setItem(row,4,cos)
				self.tablatarifas.item(row,4).setTextAlignment(4)
				self.tablatarifas.setItem(row,5,i1)
				self.tablatarifas.item(row,5).setTextAlignment(4)
				self.tablatarifas.setItem(row,6,i2)
				self.tablatarifas.item(row,6).setTextAlignment(4)
				self.tablatarifas.setItem(row,7,descuento)
				self.tablatarifas.item(row,7).setTextAlignment(4)
				row=row+1
		...
		def tarifaConfirmada(self):
			...
			if(self.prioridad!=0):
				...
				descuent=","+str(self.descuento.value())
				...
				cons="INSERT INTO \"TARIFA\" (prioridad,des_tar"+intok+fecok+horok+dsemok+",costo,descuento,estado,plaza) values ('"+str(self.prioridad)+descr+int1+int2+mes1+dia1+anio1+mes2+dia2+anio2+hora1+minuto1+hora2+minuto2+dsem2+cost+descuent+",1,"+str(plaza)+")"
				print("PK",cons)
				cur.execute(cons)
				conn.commit()
				...
				##param1 = USUARIO
				consu="insert into \"USUARIO_LOG\"(usuario,log,detalle,fecha) values("+str("2")+",7,'"+str(idtar)+"','"+fecha+"')"
				cur.execute(consu)
				conn.commit()
				self.cambia(11)
				self.llenaTabla()
		...
		def montos(self):
			...
			global plaza,localidad,costo2,val,pagoCompletado,contador,w,conteoPantallaPrincipal,inicioPago,imprime,cambiaColor,nom,loc,nivelDeCambio,cambio,leido,total,aux_cambio,aux_cambio1,pagado,config,monedas,monedasTotal,dineroTotal,avis,dineroTotalB,billetesTotales,billetes,tarifaVoluntaria,mensajeBoletoUsado,mensajeBoletoPerdido,mostrarTiempoDeSalidaRestante,mensajeTolerancia,mensajeAyuda
			#self.cambio.display(aux_cambio)
			#self.cambia(1)

			self.lcobrar.setText("$"+str(aux_tarifa))
			self.lsegundoCosto.setText("$"+str(costo2))
			self.lcosto1.setText("$"+str(aux_tarifa))
			self.lcosto2.setText("$"+str(costo2))
			
			#self.lcine.setText("$"+str(aux_tarifa2))
			self.lcine.setText("$"+str(aux_tarifa2))
			self.valvol.setText("$"+str(val))
			self.valvol3.setText("$"+str(val))
			#self.fol.setText(fo)
			#self.pen.setText(pe)
			self.he.setText(hh)
			self.he3.setText(hh)
			self.hs.setText(hsalida)
			self.hs3.setText(hsalida)
			aya=""
			aya=str(aux_dif).split(".",1)
			aya=aya[0]
			self.ttotal.setText(aya)
			self.ttotal3.setText(aya)
		...
		def contadorSegundos(self):
			global costo2,val,contador,tiempoTeclado,tt,ma,preguntarPorEstado,accesoAcaja,c,conteoPantallaPrincipal,aux_cambio,cambio,total,w,killer,aux_tarifa2,aux_tarifa,inicioPago,tiempoAgotadoDePago,cs1,v,a,p,q,y,z,mostrarTiempoDeSalidaRestante,mensajeBoletoPerdido,mensajeBoletoUsado,mensajeTolerancia,mensajeAyuda
		...
		def cambia(self,num):
			...
			#self.lll.setPixmap(QPixmap("/home/pi/Documents/eum/app/iconos/logo2.png"))
			#lll.setPixmap(QPixmap("/home/pi/Documents/eum/app/iconos/logo2.png"))
	...
	win = QWidget()
	l1 = QLabel()
	l1.setPixmap(QPixmap("/home/pi/Documents/eum/app/iconos/logo.png"))
...
def calculaTarifa(tiempoEstacionado,descuento):
	global tarifa,aux_tarifa,aux_tarifa2,aux_tarifa1,aux_dif,tarifaVoluntaria,tarifaSeleccionada,costo2
	aplicaDescuento=0
	indicador=0
	costillo=0
	horasRestantes=0
	...
	if(costillo!=0):
		...
		#aux_tarifa=0
		tarifaVoluntaria=1
	print(aux_tarifa,"costo2:",costo2)
	print("respuesta: ")
...
def leerArchivo():
	global habilitaMonedero,costo2,contador,cobroIniciado,tolerancia,pagado,preguntarPorEstado,leido,fo,pe,fe,hh,hsalida,kill,killer,killbill,config,fechaAMD,nivelDeCambio,h,nivelActual,aux_tarifa,aux_tarifa2,imprime,NoCajero,tarifaSeleccionada,mostrarTiempoDeSalidaRestante,mensajeBoletoPerdido,mensajeBoletoUsado,tarifasAplicadas,mensajeTolerancia
	A=0
	#result=0
	while(kill == 0):
		#if(imprime==1):
...
def billf():
	global costo2,cp,tiempoAgotadoDePago,cambiaColor,total,bill,cambio,tarifa,aux_cambio,aux,rep,estatus,ser,killbill,aux_tarifa,aux_tarifa2,bill,pagado,billetesTotales,dineroTotal,billetes,dineroTotalB,billetesPago
...
def leerCodQR():
	...
	try:
		lee = os.system("zbarcam --raw --prescale=280x150  /dev/video0 > /home/pi/Documents/eum/app/caseta/ticket.txt")
...
def my_callback(channel):
	global habilitaMonedero,contador,aux_tarifa,val,aux_tarifa2
