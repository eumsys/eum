...
#			self.bimpReporteCaja.clicked.connect(self.imprimeReporte)
			...
			self.bsellado.clicked.connect(lambda:self.abrirBarrera(9))
	...
	################MODS########
		...
		def abrirBarrera(self,tipo):
			global cobroIniciado,pagado,tarifaSeleccionada,NoCajero,pe,fe,costillo,hh,fo,mensajeBoletoUsado,aux_tarifa,aux_tarifa2,segundoCosto
			pagado=2
			horaHoy=time.strftime("%H:%M:%S")
			fecha=datetime.now().date()
			...
			if(tipo==9):
				print("aaaa",aux_tarifa2,segundoCosto)
				if(aux_tarifa2<segundoCosto):
					tipo=10
				segundoCosto = aux_tarifa2
				if(result==1):
					print("FOLIO YA REGISTRADO")
					mensajeBoletoUsado=1	
				else:
					self.volConfirmado()
					consu="insert into \"BOLETO\" (folio,tipo,costo,tarifa,cajero,estado,pago,expedidora,\"fechaExpedicion\",expedido) values("+str(fo)+","+str(tipo)+","+str(segundoCosto)+","+str(tarifaSeleccionada)+","+str(NoCajero)+",2,1,'"+str(pe)+"','"+fec+" "+horaHoy+"','"+fecBoleto+"')"
					#consu="insert into \"BOLETO\"(tipo,costo,tarifa,cajero,estado,pago,expedidora,\"fechaExpedicion\") values("+str(tipo)+",100,"+str(tarifaSeleccionada)+","+str(NoCajero)+",2,1,'"+str(pe)+"','"+fe+" "+horaHoy+"')"
					print(consu)
					cur.execute(consu)
					conn.commit()
		...
		def volConfirmado(self):
			global aux_tarifa,segundoCosto
		...
		def montos(self):
			...
			self.lsegundoCosto.setText("$"+str(segundoCosto))
			self.lsegundoCosto2.setText("$"+str(segundoCosto))
...
def calculaTarifa(tiempoEstacionado,descuento):
	global tarifa,aux_tarifa,aux_tarifa2,aux_tarifa1,aux_dif,tarifaVoluntaria,tarifaSeleccionada,segundoCosto
	aplicaDescuento=0
	indicador=0
	costillo=0
	horasRestantes=0
	...
	if(costillo!=0):
		...
		#aux_tarifa=0
		tarifaVoluntaria=1
	print(aux_tarifa,"segundoCosto:",segundoCosto)
	print("respuesta: ")
...
def leerArchivo():
	global segundoCosto,contador,cobroIniciado,tolerancia,pagado,preguntarPorEstado,leido,fo,pe,fe,hh,hsalida,kill,killer,killbill,config,fechaAMD,nivelDeCambio,h,nivelActual,aux_tarifa,aux_tarifa2,imprime,NoCajero,tarifaSeleccionada,mostrarTiempoDeSalidaRestante,mensajeBoletoPerdido,mensajeBoletoUsado,tarifasAplicadas,mensajeTolerancia
	A=0
	#result=0
	while(kill == 0):
		#if(imprime==1):
...
def billf():
	global segundoCosto,cp,tiempoAgotadoDePago,cambiaColor,total,bill,cambio,tarifa,aux_cambio,aux,rep,estatus,ser,killbill,aux_tarifa,aux_tarifa2,bill,pagado,billetesTotales,dineroTotal,billetes,dineroTotalB,billetesPago
...
def my_callback(channel):
	global contador,aux_tarifa,val,aux_tarifa2
