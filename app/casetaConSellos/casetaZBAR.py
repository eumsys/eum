...
#			self.bimpReporteCaja.clicked.connect(self.imprimeReporte)
		...
		def borrando(self,sino):
			global c,aux_cambio,total,w,killer,aux_tarifa
			dia=str(self.fechaborrado.date().day())
			mes=str(self.fechaborrado.date().month())
			anio=str(self.fechaborrado.date().year())
		...
		def ticketsino(self,sino):
			...
			if(sino == 1):
				...
				leerArch.write(fec)
				leerArch.close()
				os.system("sudo python3 /home/pi/Documents/eum/caseta/archimp.py")
		...
		def abrirBarrera(self,tipo):
			global pagado,tarifaSeleccionada,NoCajero,pe,fe,costillo,hh,fo,mensajeBoletoUsado
			pagado=2
		...
		def cancelandoPago(self):
			global cp,aux_tarifa,aux_tarifa1,total,killer
			aux_tarifa = 0
			aux_tarifa1 = 0
			total = 0
			aux_tarifa=0
		...
		def contadorSegundos(self):
			global ma,preguntarPorEstado,accesoAcaja,c,conteoPantallaPrincipal,aux_cambio,cambio,total,w,killer,aux_tarifa,inicioPago,tiempoAgotadoDePago,cs1,v,a,p,q,y,z,mostrarTiempoDeSalidaRestante,mensajeBoletoPerdido,mensajeBoletoUsado,mensajeTolerancia,mensajeAyuda
...
def calculaTarifa(tiempoEstacionado,descuento):
	...
	if(descuento==1):
		...
		cur.execute("select * from \"TARIFA\" where estado=1 and descuento=1 order by prioridad Asc")
	else:
...
def leerArchivo():
	global tolerancia,pagado,preguntarPorEstado,leido,fo,pe,fe,hh,hsalida,kill,killer,killbill,config,fechaAMD,nivelDeCambio,h,nivelActual,aux_tarifa,imprime,NoCajero,tarifaSeleccionada,mostrarTiempoDeSalidaRestante,mensajeBoletoPerdido,mensajeBoletoUsado,tarifasAplicadas,mensajeTolerancia
	A=0
	#result=0
	while(kill == 0):
		#if(imprime==1):
		...
		while(killer == 0 and kill == 0):
			...
			if(folio != ''):
				...
				#booleana=str("unicos de Mexico") in str(folio)
				print(booleana)
				if(booleana):
					print(folio)
...
def leerCodQR():
	...
	try:
		...
		lee = os.system("zbarcam --raw  --prescale=10x10 /dev/video0 > ticket.txt")
	except e:
		mensajeTolerancia=1
