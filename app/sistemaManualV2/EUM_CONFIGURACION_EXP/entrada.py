...
from configDateTime import dConfDT
from vserial import dvserial
...
class Vista(QtGui.QWidget):
    ...
    def initUI(self):
        ...
        #Interfaz de Fecha y Hora UPDATE SISTEM
        self.configDT = QtGui.QDialog(self.Form)
        self.conDT= dConfDT(self.configDT)
        #Interfaz de numero de serie y modelo
        self.serieModelo = QtGui.QDialog(self.Form)
        self.serMod = dvserial(self.serieModelo)
    ...
    def secretAccess(self):
        ...
        elif palabraMagica==SUPER_PASSWORD_FECHAHORA and indexCombo==3:
            self.accesoT.setHidden(True)    #Hacemos la ventana de acceso invisible
            self.configDT.show()
            self.conDT.pushButton_Aceptar.clicked.connect(self.logicaConfiguracionDT)
            self.conDT.pushButton_Cancelar.clicked.connect(self.botonCancelarConfiguracionDT)
        elif indexCombo == 4:
            self.accesoT.setHidden(True)
            self.serieModelo.show()
            self.serMod.pushButton_2.clicked.connect(self.botonCancelarSerieModelo)
        elif palabraMagica==SUPER_PASSWORD_APAGAR and indexCombo==5:
            self.accesoT.setHidden(True)    #Hacemos la ventana de acceso invisible
    ...
    def botonCancelarConfiguracionDT(self):
        self.accesoT.show()
        self.aT.palabraMagica.setText("") #Limpiamos
        self.configDT.setHidden(True)
        pass
    def botonCancelarSerieModelo(self):
        self.accesoT.show()
        self.aT.palabraMagica.setText("") #Limpiamos
        self.serieModelo.setHidden(True)
        pass

    def logicaConfiguracionDT(self):
        print "ACTIALIZANDO LA FECHA Y HORA"
        date_objet=self.conDT.dateTimeEdit.dateTime()
        dt=str(date_objet.toString("yyyy-MM-dd hh:mm:ss"))
        formato="\""+dt+"\""
        print formato
        os.system("sudo date -s "+formato)
        reiniciarSistemaUpdate(self)
...
def reiniciarSistemaUpdate(self):
