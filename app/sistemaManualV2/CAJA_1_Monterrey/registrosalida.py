...
class Ui_Form(object):
	...
	def lostTicket(self):
		...
		self.Total = (calTar.calcularMonto(0,4,0))
	...
	def calcularMinutos(self):
		...
		self.Total = calTar.calcularMonto(self.tmins, self.itarifa, 0)
	...
	def calcularMinutosParaLeerBoletos(self,horaIn, minutosIn):
		...
		if self.tmins<0:
			self.tmins=self.tmins*(-1)
			print "ERROR HORA DE SALIDA MAYOR A LA DE ENTRADA"
		self.Total,tipoRegistro = calTar.calcularMonto(self.tmins, self.itarifa, 0)
		if tipoRegistro==4:
			self.treg=4
		elif tipoRegistro==5:
			self.treg=5
