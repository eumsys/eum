...
import time
import sched
import conexionBD as coneccion
...
def interface():
	class Ventana(QDialog):
		def __init__(self):
			...
			gui = uic.loadUi("/home/pi/Documents/eum/app/sistemaManualV2/Caja/Interfaces/kiosco.ui", self) #Se utiliza la ruta completa para el boot
			self.cambia(0)
			self.loopFunction()
		...
		def tecladoSum(self,val):
			...
			if(val == 10 and var == 1):														#Borrar el ultimo caracter
				...
				self.lineContrasena.setText("")
				self.lineIdEmpleado.setText("")
				algo = coneccion.iniciarSesion(mensajeIdUser,mensajePsw)
				if(algo == "Inicio de \nSesion Exitoso"):
					self.cambia(1)
					self.label_3.setVisible(False)
				else:
...
if __name__ == "__main__":
	...
	plaza,localidad = coneccion.consultaPlazaLocalidad()
